<?php
  class MysqlColumns implements ColumnsInterface{
    private $driver;
    ...
    function getColumns(Table $table){
      if ($table->columns_changed){
        $return = array();
        $f_gc = $this->baza->query("SHOW FULL COLUMNS FROM " . $this->driverob->sep_table . $table->get("name") . $this->driverob->sep_table);
        while($d_gc = $f_gc->fetch()){
          if (!$table->columns[$d_gc["Field"]]){
            ...
            $table->columns[$d_gc["Field"]] = new Column($table, array(
                "name" => $d_gc["Field"],
                "notnull" => $notnull,
                "type" => $type,
                "maxlength" => $maxlength,
